...
<br/>
<br/>
All in all, these data suggest that DDX39B participates in DNA repair pathway via up-regulating BRCA1 at the mRNA level. <br/><br/>
...
<br/>
**Figure 2:** Heatmap of WT and KO gene expression data set. Variability at the WT vs KO. The WT and KO seem to have very different gene expression levels.
<br/>
<br/>
...
From the over-representation results, we found that the results support the conclusions made in the paper by Xu et.al. As mentioned earlier, in the paper, they analyse the downregulation of genes upon the knockout of DDX39B gene. They saw that upon the knockout of DDX29B, many of the genes corresponding to the DNA repair pathway, including genes like BRCA1 are significantly downregulated. This means that the knockout of these genes will prevent the cell from being able to repair its damaged DNA, which make the cell vulnerable to chemotherapeutic agents. From our over-representation analysis using G:profiler, we found that the downregulated genes, were over-represented in the pathways that correspond to DNA repair and regulation which supports the mechanism discussed in the paper. 

All in all, it is evident that the over-representation results show that the downregulated genes have correspondence to DNA repair and organization pathways. 
<br/>


In the paper by Chabalier et.al. they show that the downregualation of BRCA1 which is an improtant downregulated gene in the paper by Xu et. al. effects the chromosome related pathways including  inactivation of the spindlecheckpoint. This pathway is related to the mitotic sister chromatid segragation which is related to the inactivation of the spindle checkpoint as seen in the G:profiler results. Hence, the downregualted genes from my over-representation results do in fact show the downregulation of genes in a similar pathway as BRCA1 which have functions in DNA repair. 
<br/>
...
I downloaded Bader lab data. It includes the Reactome, Go, and wikipathways database gene sets which are pulled regularly. So I will use these as my genesets because they are up to date.
...
These contain collection from March 1, 2021 containing GO biological process, and no IEA and pathways.
<br/>
...
From the pathways that I notice in the GSEA output results, especially the DNA Double-Strand Break Repair pathway downregulated gene-sets, are very interesting among the pathways that are shown. This pathway was analyzed in the paper by Xu et.al and was downregualted upon DDX39B knockout in their research as well. Hence, this pathway as expected, is in the downregulated genesets. 
Additionally, other pathways related to telomere organization, and BRCA1 regualtion are in the downregulated genesets. Hence, these results confirm the results from the paper by Xu et. al. that also show downregualtion of these pathways upon knockout of the DDX39B complex. 

Note that for the analysis, I filtered for a minimum of 15 genes and a maximum of 200 genes in my pathway.
...
There are 208 nodes and 1994 edges in the enrichment map network. 

I used a p-value (in scientific notation) of 5e-08 to narrow down the pathways that are considered significant. 
A node in the network represents a gene-set or pathway, and an edge represents the similarity between the gene sets.  

The blue nodes show the downregulated pathways, and are seen to have associations with DNA repair pathways as mentioned from the paper. The red nodes are upregulated and the pathway is not of significant interest with regards to the research performed by Xu et.al. 
For instance, Figure 3 indicates that the downregulated genes in the blue nodes correspond to BRCA1 downregulation and DNA double strated repair as it was mentioned in the paper. 
...
From the network results, it is evident that the up-regulated genes do not significantly map to the geneset pathways. As shown in the red nodes, there are only 3 pathways that are significantly upregulated from my gene set. Also, as expected, the down-regulated genes have many pathway nodes in the graph representation, which are the focus group of the study by Xu et.al. 
![cytoscape result](images/fig4.png)
<br/>
**Figure 4:** Geneset Network: The downregulated genesets are blue nodes and upregulated are red nodes
<br/>
<br/>


Below is Figure 4 as well as the network legend put into one figure. 

![cytoscape result](images/fig_and_leg.png)
<br/>
**Figure 5:** Geneset Network: Complete network of gene sets including the legend
<br/>
<br/>
...
Leading edge genes are within the enrichment that come before the max enrichment score. These drive this enrichment, and make this set have a high score.
...
<br/>
**Figure 6:** Figure for the different gene sets analysed. The important pathway sets are highlighted with their names. The legend shows the desciption of the graph. The DNA repair pathway is represented by the top most cluster. The DNA double stranded pathway that is of interest is highlighted with green text. 
<br/>
<br/>
...
Some themes in the downregulated genes include telomere maintanence, template DNA replication, repair recombination. These are pathways themes that are shown to be downregulated in the paper by Xu et. al. Hence, it is significant to see the group of downregulated pathways are shown to be related to the DNA repair pathway results. The meiosis DNA repair pathways are novel and are not discussed in the paper by Xu et.al. but many of the downregulated pathways correspond to meiotic pathways in this dataset.  

![cytoscape result](images/results_non_threshold.GseaPrerankedcollapse3.png)
<br/>
**Figure 7:** Theme collapsed figure for the network analysis
<br/>
<br/>
...
<br/>
**Figure 8:** Themes of interest are the donwregulated pathways relate to DNA repair. These pathways include base excision pathways, homologous hdr strand (which corresponds to DNA double stranded repair theme), Replication stress DNA, etc.
<br/>
<br/>
...
Yes, the enrichment results follow closely with the mechanism discussed in the paper. It is evident that as discussed in the paper by Xu et.al., upon knockout of the DDX39B complex, DNA break repair pathways including BRCA1 binding will be down regulated. Hence preventing the repair of the DNA upon breakage. From the enrichment analysis results, it is clear that the downregulated pathways correspond to genesets involved in DNA Double-Strand Break Repair pathway, BRCA binding with the DNA, DNA repair pathways, etc. are all downregulated in the DDX39B knockout samples. This also follows from the threshold methods in assignment 2 which the downregulated genes were again shown to be involved in DNA repair pathways. eHnce, the results from the paper by Xu et.al, enrichment results, and threshold methods all convey the same information about the downregulated genes upon DDX39B Knock out. All of these results show the downregulation of DNA repair pathways and also downregulation of BRCA1 binding to the DNA upon knowckout of the DDX39B complex.
...
I have chosen the  DNA Double-Strand Break Repair pathway which is the path number 5693532 on wikipathways. This pathway is very significant to my analysis because in the publication by Xu et.al, the KO genes of the DDX39B complex cause a downregulation of the DNA Double-Strand Break Repair pathways. Specifically, the BRCA2 genes are downregualted. 

![cytoscape result](images/fig5.png)
<br/>
**Figure 9:** Wikipathway DNA Double-Strand Break Repair pathway which is the path number 5693532 on wikipathways. This shows the effect of this pathway on the binding on DNA repair proteins as well.
<br/>
<br/>


From the geneMANIA view, overlapping the network pathway with the log expression values, BRCA1 is seen as downregulated in the pathway for DNA Double-Strand Break Repair pathway. This result again coincides with results from the paper by Xu et.al. which indicated that the KO of the DDX39B complex leads to the downregulation of the BRCA1 genes which in turn downregulates the DNA Double-Strand Break Repair pathway.

![cytoscape result](images/fig7.png)
<br/>
**Figure 10:** GeneMania of the DNA Double-Strand Break Repair pathway interaction network, overlayed with the rank data. BRCA1 is downregulated as seen in the figure, and is highlighted in Yellow.
<br/>
<br/>

![cytoscape result](images/fig6.png)
<br/>
**Figure 11:** Close up of GeneMania of the DNA Double-Strand Break Repair pathway interaction network overlayed with the rank data. BRCA1 is downregulated as seen in the figure. 
<br/>
<br/>
